...
def setup_jenkins_agent_ubuntu():
    os.makedirs(WORKDIR, exist_ok=True)                                                # Create the working directory for the Jenkins agent if it doesn't exist
    run_cmd("sudo apt update && sudo apt install -y openjdk-21-jre wget")              # Install required packages: Java 21 (runtime) and wget for downloading files
    run_cmd(f"sudo wget {JENKINS_URL}/jnlpJars/agent.jar -O {WORKDIR}/agent.jar")      # Download the Jenkins agent JAR file from the Jenkins controller
    run_cmd(f"java -jar {WORKDIR}/agent.jar "
            f"-jnlpUrl {JENKINS_URL}/computer/{AGENT_NAME}/jenkins-agent.jnlp "
            f"-secret {AGENT_SECRET} "
            f"-workDir {WORKDIR}")                                                     # Launch the Jenkins agent using the JAR file with the specified secret and work directory

# Set up Jenkins agent for a RedHat node
def setup_jenkins_agent_redhat():
    os.makedirs(WORKDIR, exist_ok=True)                                                # Create the working directory for the Jenkins agent if it doesn't exist
    run_cmd("sudo yum install -y java-21-openjdk wget")                                # Install required packages: Java 21 (runtime) and wget for downloading files
    run_cmd(f"sudo wget {JENKINS_URL}/jnlpJars/agent.jar -O {WORKDIR}/agent.jar")      # Download the Jenkins agent JAR file from the Jenkins controller
    run_cmd(f"java -jar {WORKDIR}/agent.jar "
            f"-jnlpUrl {JENKINS_URL}/computer/{AGENT_NAME}/jenkins-agent.jnlp "
            f"-secret {AGENT_SECRET} "
